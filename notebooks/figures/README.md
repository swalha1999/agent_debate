...
(`runs/dataset/runs_summary.csv`, **n = 4** debates — the full 10-round
`capitalism` run, the 5-round `four-day-work-week` policy run, the 5-round
`online-education` education debate, and the 5-round `nuclear-energy` energy
debate). They are
...
empirical basis is n = 4 runs (an economics topic at 10 rounds, a policy
topic at 5 rounds, an education topic at 5 rounds, and an energy topic at 5
rounds), so these illustrate tendencies across four debates rather than
statistical trends across many.

## 1. Who-wins distribution

![Who-wins distribution](who_wins.png)

The four committed runs split evenly: `capitalism` and `four-day-work-week` were won by
**pro**; `online-education` and `nuclear-energy` were won by **con**.
With only four runs this is a small sample — it does not establish a structural
...
**0 % convergence across all four runs** — all debates ended in disagreement
...
across all four runs is consistent with the format holding the adversarial tension
...
**0 nudges on both sides across all four runs.** The controller's drift/nudge
mechanism (§8) corrects a debater that gets captured by its opponent and
abandons its assigned side. In none of the four runs did it have to intervene: the side
...
logic is exercised directly by the engine tests (task 8.4); all four real runs
simply did not trip it. With n = 4 we read this as "the anchoring held on
