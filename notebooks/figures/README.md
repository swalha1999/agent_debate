...
(`runs/dataset/runs_summary.csv`, **n = 2** debates — the full 10-round
`capitalism` run and the 5-round `four-day-work-week` policy run). They are
...
empirical basis is n = 2 runs (an economics topic at 10 rounds and a policy
topic at 5 rounds), so these illustrate tendencies across two debates rather
than statistical trends across many.

## 1. Who-wins distribution

![Who-wins distribution](who_wins.png)

Both committed runs (`capitalism` and `four-day-work-week`) were won by **pro**.
With only two runs this is a small sample — it does not establish a structural
favourite for the pro side. The verdict mechanism itself (side-anchoring +
...
engine tests, not inferred from a two-run chart.

## 2. Agree vs disagree

![Agree vs disagree](agree_vs_disagree.png)

**0 % convergence across both runs** — both debates ended in disagreement
...
across both runs is consistent with the format holding the adversarial tension
...
**0 nudges on both sides across both runs.** The controller's drift/nudge
mechanism (§8) corrects a debater that gets captured by its opponent and
abandons its assigned side. In neither run did it have to intervene: the side
...
logic is exercised directly by the engine tests (task 8.4); both real runs
simply did not trip it. With n = 2 we read this as "the anchoring held on
both topics tested", not as proof the controller never nudges.
