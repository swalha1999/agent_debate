...
| 2 | `docs/` with `PRD.md`, `PLAN.md`, `TODO.md` | ✅ | [`docs/PRD.md`](PRD.md) (PRD). The canonical planning doc is [`docs/ARCHITECTURE.md`](ARCHITECTURE.md) (PLAN role — C4-style overview, ADR decisions table) and the canonical task list is [`docs/TASKS.md`](TASKS.md) (TODO role — epics, status, dependencies, definition of done). For the guideline's exact filenames, thin pointer stubs [`docs/PLAN.md`](PLAN.md) and [`docs/TODO.md`](TODO.md) now also exist, each linking to the canonical doc — so both the exact-filename expectation and the canonical docs are satisfied. |
